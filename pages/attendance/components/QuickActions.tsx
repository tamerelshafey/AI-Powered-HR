...
const ActionButton: React.FC<{ icon: string; titleKey: string; subtitleKey: string; subtitleParams?: { [key: string]: string | number }; iconBg: string; iconColor: string, onClick?: () => void }> = ({ icon, titleKey, subtitleKey, subtitleParams, iconBg, iconColor, onClick }) => {
    const { t } = useI18n();
    return (
        <button onClick={onClick} className="w-full flex items-center space-x-3 space-x-reverse p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors text-start">
...
const QuickActions: React.FC<QuickActionsProps> = ({ onBiometricClick }) => {
    const { t } = useI18n();
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="p-6 space-y-4">
                <ActionButton icon="fas fa-fingerprint" titleKey="page.attendance.header.biometricCheck" subtitleKey="page.attendance.biometricModal.description.idle" iconBg="bg-indigo-100" iconColor="text-indigo-600" onClick={onBiometricClick} />
